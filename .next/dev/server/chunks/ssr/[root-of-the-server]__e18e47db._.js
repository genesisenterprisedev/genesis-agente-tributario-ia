    ...
    for (const text of texts){
        try {
            ...
            if (!response.ok) {
                ...
                // Fallback: vetor de zeros com dimensão esperada
                embeddings.push(Array(EMBEDDING_DIMENSION).fill(0));
                continue;
            }
            const data = await response.json();
            const embedding = Array.isArray(data.embedding) ? data.embedding : undefined;
            if (!embedding || embedding.length !== EMBEDDING_DIMENSION) {
                console.error("/api/embed returned embedding with unexpected dimension", embedding?.length);
                embeddings.push(Array(EMBEDDING_DIMENSION).fill(0));
                continue;
            }
            embeddings.push(embedding);
        } catch (error) {
            console.error("Error calling /api/embed:", error);
            // Fallback: vetor de zeros com dimensão esperada
            embeddings.push(Array(EMBEDDING_DIMENSION).fill(0));
        }
    }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e18e47db._.js.map